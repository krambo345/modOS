...
export async function bootstrap() {
  ...
  try {
    ...
    const userData = await kernel.account.ensureUserData();
    ...
    if (!userData) {
      ...
      await kernel.system.log(
        `Loaded ${userData.packages.length} user packages.`,
        "info",
      );
    }

    await kernel.system.log(
      `bootstrapping ${variables.osName} ${variables.osVersion}`,
    );
    ...
    kernel.system.log(
      `Entered modOS rescue mode; scrolling has been unlocked. Crash reason: ${error}`,
      "warn",
    );
